...
def test_setup_logger(tmp_path, caplog):
    """Test la création d'un logger avec setup_logger."""
    log_dir = tmp_path / "logs"

    with setup_logger("test_process", config_dir=str(log_dir), use_json=False) as logger:
...
def test_get_current_logger():
    """Test la récupération du logger courant."""
    with setup_logger("test", use_json=False) as logger:

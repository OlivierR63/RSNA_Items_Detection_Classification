...
class TestLumbarDicomTFRecordDataset:
    ...
    def test_create_tf_dataset(self):
        ...
        with patch("src.core.utils.logger.get_current_logger", return_value=self.mock_logger):
            ...
            with patch("tensorflow.data.Dataset.list_files") as mock_list_files:
                # Configure the object returned by list_files
                mock_list_files.return_value.configure_mock(**{
                    'interleave.return_value.shuffle.return_value.batch.return_value.prefetch.return_value':
                        final_mock_dataset
                })
                ...
                self.dataset._tfrecord_pattern = "tests/tmp/tfrecords/*.tfrecord"
    ...
    def test_py_deserialize_and_flatten_success(self):
        ...
        with patch.object(LumbarDicomTFRecordDataset, '_deserialize_metadata', return_value={
            'study_id': 1,
            'series_id': 1,
            'instance_number': 1,
            'description': 2,
            'condition': 3,
            'nb_records': 2,
            'records': [(0, 1, 12.34, 56.78), (1, 0, 90.12, 34.56)]
        }):
            with patch.object(LumbarDicomTFRecordDataset, '_generate_tfrecord_files', return_value=None):
                self.dataset = LumbarDicomTFRecordDataset(self.mock_config, logger=self.mock_logger)

            result = self.dataset._py_deserialize_and_flatten(mock_metadata_bytes)
    ...
    def test_parse_tfrecord(self):
        ...
        with patch("tensorflow.io.parse_single_example") as mock_parse_single_example, \
            ...
            mock_parse_tensor.return_value = tf.constant([1] * 64 * 64 * 64 * 1, dtype=tf.uint16)

            # Mock reshape to return different values on successive calls
            mock_reshape.side_effect = [
                tf.ones([64, 64, 64, 1], dtype=tf.float32),  # Premier appel pour l'image
                tf.ones([25, 4], dtype=tf.float32)           # Deuxième appel pour les records
            ]
            ...
            mock_py_function.return_value = mock_header_tensors + [mock_records_flat]

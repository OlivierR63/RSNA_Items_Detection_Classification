...
class TestTFRecordFilesManager():
    ...
    def test_generate_tfrecord_files_nominal(
        ...
        with ExitStack() as stack:
            ...
            mock_csv.return_value.merged = mock_metadata
            ...
            stack.enter_context(patch.object(mock_tfrecord_files_manager, '_get_series_stats')).return_value=(0, 256)

            # Mock calculate_max_series_depth
            depth_func_path = (
                'src.projects.lumbar_spine.tfrecord_files_manager'
                '.calculate_max_series_depth'
            )
            stack.enter_context(patch(depth_func_path)).return_value = 5
            ...
            # Total expected calls: (3*3) + (3*4) + (3*5) = 36
            assert mock_writer_instance.write.call_count == 36

            # Verify the maximum number of files per series ("depth"):
            assert mock_tfrecord_files_manager.get_max_series_depth() == 5
    ...
    def test_generate_tfrecord_files_empty_state(
        ...
        with ExitStack() as stack:
            ...
            mock_tfrecord_files_manager._config["dicom_studies_dir"] = empty_studies_dir
            ...
            # Check he logs (Pytest / caplog style)
            ...
            assert exception_msg == f"No studies found in {empty_studies_dir}. Process stops immediately."
...
class TestEncodeDataFrame:
    ...
    def test_encode_dataframe_success(self, mock_tfrecord_files_manager, mock_metadata, caplog, tmp_path):
        """
        Test successful encoding of all target columns.
        """
        # Define expected columns to be processed
        target_columns = ["condition", "level", "description", "severity"]
        ...
        with ExitStack() as stack:
            ...
            mock_create = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_create_mappings', return_value=mock_mappings))
            mock_apply = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_apply_encodings', return_value=encoded_df))
            
            result = mock_tfrecord_files_manager._encode_dataframe(mock_metadata)
    ...
    def test_encode_dataframe_missing_column(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
        ...
        with ExitStack() as stack:
            ...
            with pytest.raises(KeyError):
                mock_tfrecord_files_manager._encode_dataframe(incomplete_df)
    ...
    def test_encode_dataframe_empty_input(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
        Test behavior with an empty DataFrame (but with correct columns).
        """
        empty_df = pd.DataFrame(columns=["condition", "level", "description", "severity"])
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(mock_tfrecord_files_manager, '_create_mappings', return_value={}))
            stack.enter_context(patch.object(mock_tfrecord_files_manager, '_apply_encodings', return_value=empty_df))

            result_df = mock_tfrecord_files_manager._encode_dataframe(empty_df)
            
            assert result_df.empty
            assert list(result_df.columns) == ["condition", "level", "description", "severity"]

    def test_encode_dataframe_exception_handling(self, mock_tfrecord_files_manager, mock_metadata, caplog, tmp_path):
        """
        ...
        with ExitStack() as stack:
            ...
            with pytest.raises(RuntimeError) as exc_info:
                mock_tfrecord_files_manager._encode_dataframe(mock_metadata)
            
            # Check the exception message itself
            assert "Unexpected error" in str(exc_info.value)

            # Verify the formatted error message in logs
            expected_log_msg = "Error in function TFRecordFilesManager._encode_dataframe: Unexpected error"
...
class TestCreateMappings:
    ...
    def test_create_mappings_success(self, mock_tfrecord_files_manager, mock_metadata, caplog, tmp_path):
        """
        Nominal case: verify that mappings are created for all requested columns.
        """
        # Define columns to encode
        columns_to_encode = ["condition", "severity"]
        ...
        with ExitStack() as stack:
            ...
            mock_method = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_create_string_to_int_mapper', return_value=mock_mapper))
            result = mock_tfrecord_files_manager._create_mappings(mock_metadata, columns_to_encode)

            # Check result structure
            assert isinstance(result, dict)
            assert "condition" in result
            assert "severity" in result
            assert result["condition"] == {"dummy_key": 0}
    ...
    def test_create_mappings_sorting_and_uniqueness(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
        ...
        with ExitStack() as stack:
            ...
            mock_method = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_create_string_to_int_mapper', return_value=mock_mapper))
            mock_tfrecord_files_manager._create_mappings(df, columns)
    ...
    def test_create_mappings_missing_column_warning(self, mock_tfrecord_files_manager, mock_metadata, caplog, tmp_path):
        """
        ...
        columns_to_encode = ["condition", "unknown_column"]
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(mock_tfrecord_files_manager, '_create_string_to_int_mapper', return_value=mock_mapper))
            result = mock_tfrecord_files_manager._create_mappings(mock_metadata, columns_to_encode)
    ...
    def test_create_mappings_exception_handling(self, mock_tfrecord_files_manager, mock_metadata, caplog, tmp_path):
        """
        ...
        with ExitStack() as stack:
            ...
            with pytest.raises(Exception) as exc_info:
                mock_tfrecord_files_manager._create_mappings(mock_metadata, columns)
            ...
            assert "while processing column 'condition'" in caplog.text
...
class TestApplyEncodings:
    ...
    def test_apply_encodings_success(self, mock_tfrecord_files_manager, mock_metadata):
        """
        ...
        columns_to_encode = ["condition", "severity"]
        mappings = {
            "condition": {val: idx for idx, val in enumerate(mock_metadata["condition"].unique())},
            "severity": {val: idx for idx, val in enumerate(mock_metadata["severity"].unique())}
        }

        result = mock_tfrecord_files_manager._apply_encodings(mock_metadata, columns_to_encode, mappings)

        # Verifications
        assert result["condition"].dtype in [np.int32, np.int64]
    ...
    def test_apply_encodings_preserves_original_df(self, mock_tfrecord_files_manager, mock_metadata):
        """
        ...
        columns = ["condition", "severity"]

        # Simple mapping to force a change in the result
        mappings = {
            "condition": {val: 1 for val in df_input["condition"].unique()},
            "severity": {val: 1 for val in df_input["severity"].unique()}
        }

        # Act
        result_df = mock_tfrecord_files_manager._apply_encodings(df_input, columns, mappings)
        ...
        assert df_input.loc[0, "condition"] == df_input_original.loc[0, "condition"]
        assert isinstance(df_input.loc[0, "condition"], str)

    def test_apply_encodings_with_unknown_values(self, mock_tfrecord_files_manager):
        """
        Verify that values not present in the mapping are filled with -1.
        """
        df = pd.DataFrame({"condition": ["Known", "Unknown"]})
        columns = ["condition"]
        mappings = {"condition": {"Known": 5}}

        result = mock_tfrecord_files_manager._apply_encodings(df, columns, mappings)

        assert result["condition"].tolist() == [5, -1]
        assert np.issubdtype(result["condition"].dtype, np.integer)

    def test_apply_encodings_with_nans(self, mock_tfrecord_files_manager):
        """
    ...
    def test_apply_encodings_partial_columns(self, mock_tfrecord_files_manager):
        """
...
class TestCreateStringToIntMapper:
    ...
    def test_mapper_creation_success(self, mock_tfrecord_files_manager):
        """
        ...
        with patch('src.core.utils.logger.get_current_logger') as mock_get_log:
            mock_get_log.return_value = mock_tfrecord_files_manager._logger # Reuse the mock_logger from the manager

            # Generate the mapper function
            mapper = mock_tfrecord_files_manager._create_string_to_int_mapper(pathologies)
    ...
    def test_mapper_with_empty_list(self, mock_tfrecord_files_manager):
        """
        ...
        with patch('src.core.utils.logger.get_current_logger') as mock_get_log:
            ...
            mapper = mock_tfrecord_files_manager._create_string_to_int_mapper([])

        assert mapper("Any") == -1
        assert mapper.mapping == {}
        assert mapper.reverse_mapping == {}

    def test_mapper_idempotency_and_order(self, mock_tfrecord_files_manager):
        """
        ...
        with patch('src.core.utils.logger.get_current_logger') as mock_get_log:
            ...
            mapper = mock_tfrecord_files_manager._create_string_to_int_mapper(input_list)
    ...
    def test_mapper_exception_handling(self, mock_tfrecord_files_manager, caplog):
        """
        ...
        with patch('src.core.utils.logger.get_current_logger') as mock_get_log:
            ...
            # Passing None will cause enumerate() to raise a TypeError
            with pytest.raises(TypeError):
                mock_tfrecord_files_manager._create_string_to_int_mapper(None)
...
class TestConvertDicomToTFrecords:
    ...
    # ---  Nominal case ---
    def test_convert_dicom_to_tfrecords_nominal(self, mock_tfrecord_files_manager, mock_config, mock_encoded_metadata, caplog, tmp_path):
        # Use real structure created by setup_dicom_tree_structure
        studies_dir = mock_config["dicom_studies_dir"]
        tfrecord_dir = mock_config["tfrecord_dir"]
        ...
        with ExitStack() as stack:
            ...
            mock_setup_dir = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_setup_tfrecord_directory'))
            mock_setup_dir.return_value = Path(tfrecord_dir)
            
            mock_process = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_process_study'))
            
            mock_tfrecord_files_manager._convert_dicom_to_tfrecords(studies_dir, mock_encoded_metadata, str(tfrecord_dir))
    ...
    # --- Empty root folder ---
    def test_convert_dicom_to_tfrecords_empty_root(self, mock_tfrecord_files_manager, mock_encoded_metadata, caplog, tmp_path):
        ...
        with patch('src.core.utils.logger.get_current_logger') as mock_get_log:
            ...
            with pytest.raises(EmptyDirectoryError, match="No studies found") as excinfo:
                mock_tfrecord_files_manager._convert_dicom_to_tfrecords(empty_dir, mock_encoded_metadata, "dummy_out")
        ...
        # Verify that the function actually stopped the process and sent the proper message to the user.
        assert exception_msg == f"No studies found in {empty_dir}. Process stops immediately."

    # --- Empty study folder (skip it) ---
    def test_convert_dicom_to_tfrecords_skips_empty_study(self, mock_tfrecord_files_manager, mock_config, mock_encoded_metadata, caplog, tmp_path):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(mock_tfrecord_files_manager, '_setup_tfrecord_directory'))
            mock_process = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_process_study'))
            
            mock_tfrecord_files_manager._convert_dicom_to_tfrecords(studies_dir, mock_encoded_metadata, "out")
            
            # Should still be 3 (the 9999 is skipped because any().iterdir is False)
            assert mock_process.call_count == 3

            assert f"Skipping empty directory {empty_dir} in root directory {studies_dir}" in caplog.text

            warning_record = next(r for r in caplog.records if "Skipping empty directory" in r.message)
            assert warning_record.status == "Skipped studies"


    # --- Non folder item (skip it) ---
    def test_convert_dicom_to_tfrecords_skips_file(self, mock_tfrecord_files_manager, mock_config, mock_encoded_metadata, caplog, tmp_path):
    ...
    # --- Missing metadata ---
    def test_convert_dicom_to_tfrecords_missing_metadata(self, mock_tfrecord_files_manager, mock_config, caplog, tmp_path):
        ...
        with ExitStack() as stack:
            ...
            mock_process = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_process_study'))
            mock_tfrecord_files_manager._convert_dicom_to_tfrecords(studies_dir, partial_metadata, "out")
    ...
    def test_convert_dicom_to_tfrecords_exception(self, mock_tfrecord_files_manager, mock_config, mock_encoded_metadata, caplog, tmp_path):
        studies_dir = mock_config["dicom_studies_dir"]
        ...
        with ExitStack() as stack:
            ...
            # Access the string represetation of the exception
            exception_msg = str(excinfo.value)
...
class TestSetupTfrecordDirectory:
    ...
    def test_setup_tfrecord_directory_creates_new_folder(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_setup_tfrecord_directory_already_exists(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_setup_tfrecord_directory_nested_creation(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_setup_tfrecord_directory_path_is_a_file(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_setup_tfrecord_directory_permission_denied(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
...
class TestProcessStudy:
    ...
    def test_process_study_nominal(
        ...
        with ExitStack() as stack:
            stack.enter_context(patch('src.core.utils.logger.get_current_logger', return_value=mock_tfrecord_files_manager._logger))
            stack.enter_context(patch('tensorflow.io.TFRecordWriter'))
            ...
            mock_p.side_effect = [(3, 0, 3), (4, 0, 4), (3, 0, 3)]
            
            mock_tfrecord_files_manager._process_study(study_path, mock_encoded_metadata, studies_dir / "out")
    ...
    def test_process_study_skips_files(
        ...
        with ExitStack() as stack:
            stack.enter_context(patch('src.core.utils.logger.get_current_logger', return_value=mock_tfrecord_files_manager._logger))
            stack.enter_context(patch('tensorflow.io.TFRecordWriter'))
            stack.enter_context(patch.object(TFRecordFilesManager, '_get_series_stats', return_value=(0, 255)))
            stack.enter_context(patch.object(TFRecordFilesManager, '_process_single_series_instance', return_value=(3, 0, 3)))
            
            mock_tfrecord_files_manager._process_study(study_path, mock_encoded_metadata, studies_dir / "out")
            
            # Verify warning for non-directory item
            assert any("Skipping non-directory" in rec.message for rec in caplog.records if rec.levelname == "ERROR")

    # 3. --- Skips Empty Series Dirs ---
    def test_process_study_skips_empty_series_dirs(
        self,
        mock_tfrecord_files_manager,
        setup_dicom_tree_structure,
        mock_encoded_metadata,
        ...
        """
        Verify that directories containing no files are identified and skipped.
        
        This test ensures that:
        1. The function checks for content within each series directory.
        2. Empty directories trigger a specific warning log to prevent empty TFRecord entries.
        3. The 'extra' metadata (status='Skipped studies') is associated with the log record.
        """

        studies_dir = setup_dicom_tree_structure
        study_path = studies_dir / "1010"
        (study_path / "9999").mkdir() # Empty series directory
        
        with ExitStack() as stack:
            stack.enter_context(patch('src.core.utils.logger.get_current_logger', return_value=mock_tfrecord_files_manager._logger))
            stack.enter_context(patch('tensorflow.io.TFRecordWriter'))
            mock_process = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_process_single_series_instance'))
            mock_process.side_effect = [(3, 0, 3), (5,0,0), (4, 0, 4), (0, 0, 0)]
            
            with pytest.raises(SeriesProcessingError, match="skipped series"):
                mock_tfrecord_files_manager._process_study(study_path, mock_encoded_metadata, studies_dir / "out")
            
            # Verify that all the series in the study were visited
            assert mock_process.call_count == 4

            # Verify warning for empty directory
            record = next(rec for rec in caplog.records if "processed with 1 skipped series" in rec.message)
            assert record.levelname == "ERROR"
            assert record.status == "failure"
            assert record.nb_series == 4
            assert record.skipped_series == 1

    # 4. --- Writer Error (Disk Full) ---
    def test_process_study_writer_error(
        ...
        with ExitStack() as stack:
            stack.enter_context(patch('src.core.utils.logger.get_current_logger', return_value=mock_tfrecord_files_manager._logger))
            stack.enter_context(patch('tensorflow.io.TFRecordWriter', side_effect=IOError("Disk Full")))
            
            # Track if processing started despite the early writer error
            mock_p = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_process_single_series_instance'))

            with pytest.raises(IOError, match="Disk Full"):
                mock_tfrecord_files_manager._process_study(study_path, mock_encoded_metadata, studies_dir / "out")

       # Validate the specific error record exists
        error_record = next(rec for rec in caplog.records if rec.levelname == "ERROR")
    ...
    # 5. --- Empty Study Directory ---
    def test_process_study_empty_study_dir(self, mock_tfrecord_files_manager, setup_dicom_tree_structure, mock_encoded_metadata, caplog):
        """
        ...
        with ExitStack() as stack:
            ...
            with pytest.raises(EmptyDirectoryError, match="is empty"):
                mock_tfrecord_files_manager._process_study(empty_study_path, mock_encoded_metadata, studies_dir / "out")
            
            # Target the specific log for empty studies
            record = next(rec for rec in caplog.records if "Study 9999 is empty" in rec.message and hasattr(rec, 'status'))
            assert record.status == "failure"
    ...
    def test_process_study_general_exception(
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(Path, 'iterdir', side_effect=RuntimeError("Unexpected OS Error")))
...
class TestProcessSingleSeriesInstance:
    ...
    def test_process_single_series_success(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        ...
        mask_1 = ['study_id', 'series_id', 'description']
        ...
        with ExitStack() as stack:
            ...
            mock_final = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_process_series', return_value=(3, 0, 3)))
            nb_success, nb_failures, nb_files = mock_tfrecord_files_manager._process_single_series_instance(
                series_dir, input_features_df, labels_df, mock_writer
            )
    ...
    def test_process_single_series_missing_metadata(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        ...
        input_features_df = metadata_df[['study_id', 'series_id', 'description']].drop_duplicates()  # Force removal
        
        labels_df = mock_encoded_metadata[['condition_level', 'severity', 'x', 'y']].drop_duplicates() 
    ...
    def test_process_single_series_is_not_dir(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        ...
        with patch('src.core.utils.logger.get_current_logger') as mock_get_log:
            mock_get_log.return_value = mock_tfrecord_files_manager._logger
            nb_success, nb_failures, nb_files =  mock_tfrecord_files_manager._process_single_series_instance(
                fake_series_path, input_features_df, labels_df, mock_writer
            )
        ...
        assert any("Skipping non-directory item" in rec.message 
           for rec in caplog.records 
           if rec.levelname == "WARNING"
        )

    def test_process_single_series_invalid_name(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
...
class TestProcessSeries:
    ...
    def test_process_series_full_success(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        Test the case where all DICOM files in a series are processed successfully.
        Verifies that the returned tuple matches (3, 0, 3) for the 10110 series.
        """
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(mock_tfrecord_files_manager, '_get_series_stats', return_value=(0, 255)))
            
            # Mock individual DICOM processing to always succeed (return_value=True)
            mock_single = stack.enter_context(
                patch.object(mock_tfrecord_files_manager, '_process_single_dicom_instance', return_value=True)
            )
            ...
            success, failures, total = mock_tfrecord_files_manager._process_series(series_dir, input_features_df, labels_df, mock_writer)
            ...
            start_log = next(
                (rec for rec in caplog.records if "processing completed successfully" in rec.message), 
                None  # Means: start_log = None if the expected log is missing
            )

            # Ensure the record was actually found before asserting attributes
            assert start_log is not None, f"Success log for series {series_dir.name} was not found in caplog"
            assert start_log.levelname == "INFO"
            assert start_log.status == "success"


    def test_process_series_partial_success(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        Test the case where 1 file fails and 2 succeed.
        Verifies that the returned tuple is (2, 1, 3).
        """
        series_dir = tmp_path / "root/dicom/1010/10110" # 3 files
        ...
        with ExitStack() as stack:
            ...
            # Mock side_effect: first file fails (False), next two succeed (True)
            mock_single = stack.enter_context(
                patch.object(
                    mock_tfrecord_files_manager,
            ...
            success, failures, total = mock_tfrecord_files_manager._process_series(
                series_dir,
                input_features_df,
                labels_df,
                mock_writer
    ...
    def test_process_series_complete_failure(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        Test the case where all files fail (3/3).
        Verifies that the returned tuple is (0, 3, 3).
        """
        series_dir = tmp_path / "root/dicom/1010/10110"
        ...
        with ExitStack() as stack:
            ...
            mock_single = stack.enter_context(
                patch.object(mock_tfrecord_files_manager, '_process_single_dicom_instance', return_value=False)
            )
            ...
            error_record = next(
                (rec for rec in caplog.records if error_msg in rec.message and rec.levelname == "ERROR"),
                None
            )
    ...
    def test_process_series_no_dicom_files(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        ...
        empty_series = tmp_path / "root/dicom/empty_series"
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch.object(mock_tfrecord_files_manager, '_get_series_stats', return_value=(0, 0)))
            mock_single = stack.enter_context(
    ...
    def test_process_series_general_exception(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        Verify that an unexpected exception during series processing is caught, logged, and re-raised.
        """
        series_dir = tmp_path / "root/dicom/1010/10110"
...
class TestGetSeriesStats:
    ...
    def test_get_series_stats_nominal_aggregation(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_get_series_stats_empty_directory_raises_error(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_get_series_stats_corrupt_dicom_raises_exception(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_get_series_stats_accepts_string_path(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_get_series_stats_general_exception(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
        Verify that any unexpected exception is caught, logged with traceback, and re-raised.
        """
...
class TestProcessSingleDicomInstance:
    ...
    def test_process_single_dicom_success(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
    ...
    def test_process_single_dicom_extraction_failure(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        ...
        with ExitStack() as stack:
            ...
            result = mock_tfrecord_files_manager._process_single_dicom_instance(
                dicom_path, 0, 1000, input_features_df, labels_df, mock_writer
            )
    ...
    def test_process_single_dicom_write_failure(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
    ...
    def test_process_single_dicom_with_custom_logger(self, mock_tfrecord_files_manager, mock_encoded_metadata, mock_writer, caplog, tmp_path):
        """
        Verifies that a custom logger passed as argument is used instead of the default one.
        """
        custom_logger = MagicMock()
        dicom_path = Path("/mock/path/error.dcm")
        ...
        with ExitStack() as stack:
            ...
            mock_tfrecord_files_manager._process_single_dicom_instance(
                dicom_path, 0, 1000, input_features_df, labels_df, mock_writer,
    ...
    def test_process_single_dicom_instance_general_exception(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
        ...
        with ExitStack() as stack:
            ...
            result = mock_tfrecord_files_manager._process_single_dicom_instance(
                dicom_path, 0, 255, pd.DataFrame(), pd.DataFrame(), mock_writer
            )
...
class TestProcessDicomFileWithMetadata:
    ...
    def test_process_dicom_success(self, mock_tfrecord_files_manager, mock_encoded_metadata, caplog, tmp_path):
        """
        Test the successful conversion of a DICOM file to a tf.train.Example.
        """
        # Setup mock file structure and metadata
        dicom_path = Path("/root/1010/10110/555.dcm")
        series_id = 10110
        study_id = 1010
    ...
    def test_process_dicom_missing_description_error(self, mock_tfrecord_files_manager, mock_encoded_metadata, caplog, tmp_path):
        """
        Test that a ValueError is raised when the key 'description' is missing for the series.
        """
        dicom_path = Path("/root/1010/99999/1.dcm") # Series 99999 not in mock_encoded_metadata
        ...
        with ExitStack() as stack:
            ...
            with pytest.raises(ValueError) as exc_info:
                mock_tfrecord_files_manager._process_dicom_file_with_metadata(
                    dicom_path, 0, 1000, input_features_df, labels_df
                )
            
            assert "no description found" in str(exc_info.value)

            # Verify the log message contains the IDs that failed the lookup
            error_record = next((rec for rec in caplog.records if rec.levelname == "ERROR"), None)
            assert error_record is not None
            assert "no description found" in error_record.message
            assert "99999" in error_record.message # Verification of the series_id in the log

    def test_process_dicom_read_exception(self, mock_tfrecord_files_manager, mock_encoded_metadata, caplog, tmp_path):
        """
        Test that exceptions during SimpleITK read are logged and re-raised.
        """
        dicom_path = Path("/mock/root/10110/1.dcm")
    ...
    def test_process_dicom_data_type_conversion(self, mock_tfrecord_files_manager, mock_encoded_metadata, caplog, tmp_path):
        """
        Ensures that the image array is correctly cast to uint16 before byte conversion.
        """
        dicom_path = Path("/root/1010/10110/1.dcm")
        ...
        with ExitStack() as stack:
            ...
            mock_prepare = stack.enter_context(patch.object(mock_tfrecord_files_manager, '_prepare_tf_features', return_value={}))
    ...
    def test_process_dicom_file_general_exception(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch('SimpleITK.ReadImage', 
        ...
        error_record = next((rec for rec in caplog.records if rec.levelname == "ERROR"), None)
        assert error_record is not None
        assert "The DICOM file read/conversion/serialization process for 1.dcm failed" in error_record.message
        assert error_record.status == "failed"
...
class TestPrepareTfFeatures:
    ...
    def test_prepare_tf_features_fixed_positions(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_prepare_tf_features_out_of_range_selection(self, mock_tfrecord_files_manager, caplog, tmp_path):
        """
    ...
    def test_prepare_tf_features_missing_column_error(self, mock_tfrecord_files_manager, mock_logger, caplog, tmp_path):
        """

...
class TestModelTrainer:
    ...
    def mock_trainer(self, mock_config, mock_logger):
        ...
        mock_config["tfrecord_dir"].mkdir(parents=True, exist_ok=True)
    ...
    def test_prepare_datasets_logic(self, mock_trainer):
        ...
        with ExitStack() as stack:
            ...
            mock_dataset_class = stack.enter_context(patch("src.projects.lumbar_spine.model_trainer.LumbarDicomTFRecordDataset"))
            ...
            mock_ds_instance.generate_tfrecord_dataset.return_value = MagicMock(spec=tf.data.Dataset)
    ...
    def test_prepare_datasets_io_error(self, mock_trainer):
        ...
        with ExitStack() as stack:
            ...
            mock_glob = stack.enter_context(patch("src.projects.lumbar_spine.model_trainer.tf.io.gfile.glob"))
    ...
    def test_prepare_datasets_logic_failure(self, mock_trainer):
        ...
        with ExitStack() as stack:
            ...
            mock_ds_class = stack.enter_context(patch("src.projects.lumbar_spine.model_trainer.LumbarDicomTFRecordDataset"))
            ...
            mock_mem = stack.enter_context(patch("src.projects.lumbar_spine.model_trainer.log_memory_usage"))
    ...
    def test_train_model_failure_logging(self, mock_trainer):
        ...
        with ExitStack() as stack:
            ...
            mock_trainer._train_with_callbacks = MagicMock(side_effect=RuntimeError("Training failed"))
    ...
    def test_invalid_config_raises_error(self, mock_trainer):
        ...
        with ExitStack() as stack:
            ...
            mock_trainer._config["batch_size"] = 0
            mock_trainer._nb_train = 10
        
            with pytest.raises(ValueError, match="batch_size' shall be strictly positive"):
                mock_trainer._train_with_callbacks()
    ...
    def test_log_memory_usage_exception_safety(self, mock_trainer):
        ...
        with ExitStack() as stack:
            ...
            mock_vmem = stack.enter_context(patch("src.projects.lumbar_spine.model_trainer.psutil.virtual_memory"))
    ...
    def test_output_directories_creation(self, mock_trainer):
        ...
        with ExitStack() as stack:
            ...
            mock_trainer._config["output_dir"] = new_output_dir

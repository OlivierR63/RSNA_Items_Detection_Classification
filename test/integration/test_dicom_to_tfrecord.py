...
import pandas as pd
import numpy as np
from unittest.mock import patch, MagicMock, NonCallableMock, ANY
...
def mock_setup(mock_config, mock_logger):
    ...
    dicom_studies_dir = Path(mock_config["dicom_studies_dir"])
...
def test_integration_dicom_serialization(mock_setup, tmp_path):
    ...
    series_description = int(metadata_df['description'].values[0])
    ...
    with tf.io.TFRecordWriter(str(tfrecord_file)) as writer:
        ...
        with patch("SimpleITK.ReadImage", return_value=mock_image):
            ...
            with patch("SimpleITK.GetArrayFromImage", return_value=np.zeros((512, 512), dtype=np.int16)):
    ...
    for raw_record in raw_dataset.take(1):
        ...
        assert features["description"].int64_list.value[0] == series_description
